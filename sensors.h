...
5  -> PORTA5    -> ASENSOR_VBATT
6  -> PORTA6    -> ASENSOR_KNOCK
7  -> PORTA7    -> ASENSOR_BARO

8  -> PORTB0    (Decoder - CRANK)
9  -> PORTB1    (Decoder - CAM)

10  -> PORTC0   (spare in 64/100 pin devices)
11  -> PORTC1   (spare in 64/100 pin devices)
12  -> PORTC2   (spare in 64/100 pin devices)
13  -> PORTC3   (spare in 64/100 pin devices)
14  -> PORTC4   (spare in 64/100 pin devices)
15  -> PORTC5   (spare in 64/100 pin devices)
...
PORTB5  -> DSENSOR_RUN
PORTB8  -> DSENSOR_CRASH
PORTB9  -> DSENSOR_SIDESTAND
*/
#define DSENSOR_CYLINDER_ERROR_THRES 3
...
typedef enum {
    ...
    DSENSOR_RUN          =0x10,
    DSENSOR_CRASH        =0x20,
    DSENSOR_SIDESTAND    =0x40,
    DSENSOR_CYLINDER     =0x04,
...
ADC measurement average values, its counters and error counters we address
through xx[ASENSOR_yy], only MAP sensor uses map_integrator_zz for averaging.

when modifying sensors keep all indexes of regular group sensors below index of asensor_map and
*/
