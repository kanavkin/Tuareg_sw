...
# CRK_POSITION_A2 -> P.A2
printf '\x55\x00\x02\x50\x2E\x41\x32' > $DEV
sleep 0.1

# dynamic_dwell_target_us
# 6000
printf '\x55\x00\x03\x00\x00\x17\x70' > $DEV
sleep 0.1

# cold_idle_cutoff_rpm
# 1500
printf '\x55\x00\x04\x00\x00\x05\xDC' > $DEV
sleep 0.1

# cold_idle_cutoff_CLT_K
# (50 deg C) 323
printf '\x55\x00\x05\x00\x00\x01\x43' > $DEV
sleep 0.1

# cold_idle_ignition_advance_deg
# 20
printf '\x55\x00\x06\x00\x00\x00\x14' > $DEV
sleep 0.1

# cold_idle_dwell_target_us
# 10000
printf '\x55\x00\x07\x00\x00\x27\x10' > $DEV
sleep 0.1

# cranking_ignition_position
# POSITION_B2 -> P.B2
printf '\x55\x00\x08\x50\x2E\x42\x32' > $DEV
sleep 0.1

# cranking_dwell_position
# POSITION_D2 -> 7
printf '\x55\x00\x09\x50\x2E\x44\x32' > $DEV
sleep 0.1
...
# spark duration (us)
# 1500
printf '\x55\x00\x0B\x00\x00\x05\xDC' > $DEV
sleep 0.1
