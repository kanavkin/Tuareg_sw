#ifndef FUELING_SYSLOC_LOCATIONS_H_INCLUDED
#define FUELING_SYSLOC_LOCATIONS_H_INCLUDED

typedef enum {

    FUELING_LOC_CONFIG_ERROR,
    FUELING_LOC_CONFIGVERSION_ERROR,
    FUELING_LOC_CONFIG_GETCHARGETEMP_ERR,
    FUELING_LOC_READY,

    FUELING_LOC_UPDCTRL_VITAL_PRECOND,

    FUELING_LOC_UPDAIRFLOW_CHRGTEMP_INVALID,
    FUELING_LOC_UPDAIRFLOW_DENS_ERR,
    FUELING_LOC_UPDAIRFLOW_RATE_ERR,

    FUELING_LOC_UPDBASEFUEL_MASS_ERR,

    FUELING_LOC_SEQUENTIAL_CALC_FAIL,
    FUELING_LOC_INJ_DEADTIME_INVALID,

    FUELING_LOC_INJ1_DC_CLIP,
    FUELING_LOC_INJ2_DC_CLIP,
    FUELING_LOC_REL_COMP_CLIP,
    FUELING_LOC_ABS_COMP_CLIP,

    FUELING_LOC_CONF_ACCCOMP_TPS_ERROR,
    FUELING_LOC_CONF_ACCCOMP_MAP_ERROR,
    FUELING_LOC_CONF_WARMUPCOMP_ERROR,
    FUELING_LOC_CONF_INJTIMTAB_ERROR,
    FUELING_LOC_CONF_CRKFUELTAB_ERROR,
    FUELING_LOC_CONF_BAROCOMP_ERROR,



    FUELING_LOC_ACCELCOMP_RPMSCALE,
    FUELING_LOC_ACCELCOMP_LOAD_TRANS_UPD_DIV_ERROR,

    FUELING_LOC_UPDINJBEG_ENDTARGET_INVALID,

    FUELING_LOC_ACCELCOMP_CLIP_COLD_ACCEL_PCT,

    FUELING_LOC_BAROCORR_INVALID,

    FUELING_LOC_COUNT

} fueling_sysloc_locations_t;




#endif // FUELING_SYSLOC_LOCATIONS_H_INCLUDED
