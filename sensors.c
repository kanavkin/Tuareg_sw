...
volatile sensor_interface_t * init_sensors()
{
    ...
    Sensors.engine_phase= PHASE_UNDEFINED;
    ...
    //GPIO cam sensor + run + crash + sidestand
    GPIO_configure(GPIOB, 1, GPIO_IN_PUD);
    GPIO_configure(GPIOB, 5, GPIO_IN_PUD);
    GPIO_configure(GPIOB, 8, GPIO_IN_PUD);
    GPIO_configure(GPIOB, 9, GPIO_IN_PUD);
    ...
    */
    ...
    /**
    ADC channels setup
    the channel order must fit the enumeration value of
    the ASENSOR_  value in sensors_t!
    MAP sensor is in injected group
    ...
    */
    ...
    adc_set_regular_group(ADC1, 5, ASENSOR_BARO_CH);
    adc_set_regular_group(ADC1, 6, ASENSOR_VBAT_CH);
...
void read_digital_sensors()
{
    ...
    {
        Sensors.digital_sensors |= DSENSOR_SIDESTAND;
    }
    else
    {
        Sensors.digital_sensors &= ~DSENSOR_SIDESTAND;
...
/**
TODO
state machine to keep track of current engine cycle position
idea behind it: when we read the cylinder identification sensor every time the crank shaft passes a
certain position there must be a different readout every time
*/
void read_cylinder_sensor()
{
    volatile engine_phase_t new_phase;
    ...
    {
        //sensor triggered!
        //cylinder #1 at compression stroke?
        new_phase= PHASE_CYL1_COMPR;
    }
    else
    {
        new_phase= PHASE_CYL2_COMPR;
    }


    /**
    check for defective sensor (steady state)
    */
    if(Sensors.engine_phase != new_phase)
    {
        /**
        everything is fine with the sensor!
        */
        Sensors.engine_phase= new_phase;
        Sensors.active_sensors |= DSENSOR_CYLINDER_ACT;
        Sensors.cylinder_sensor_counter =0;
    }
    else
    {
        //again?!
        Sensors.cylinder_sensor_counter++;

        /**
        steady state limit reached?
        how many read events have we seen since last change?
        */
        if(Sensors.cylinder_sensor_counter > DSENSOR_CYLINDER_ERROR_THRES)
        {
            /**
            cylinder identification sensor disturbed
            */
            Sensors.engine_phase= PHASE_UNDEFINED;
            Sensors.active_sensors &= ~DSENSOR_CYLINDER_ACT;
        }
        ...
        ok for this time, lets see what will come next!
        engine_phase already set correctly
        */
